...
class CatImageList(Dataset):
    def __init__(self, dataset1, dataset2, batch_size1=27, batch_size2=27):
        super(CatImageList,self).__init__()
    ...
    def __getitem__(self, idx):
        if self.count < self.batch_size1:
            self.count += 1
            return self.dataset1[self.len1*idx//(self)],0
        else:
            self.count += 1
            return self.dataset2[self.len2*idx//self.len],1
        if self.count >= (self.batch_size1+self.batch_size2):
            self.count = 0

    def __len__(self):
        return (max(self.len1//self.batch_size1, self.len2//self.batch_size1)+1)*(self.batch_size1+self.batch_size2)
