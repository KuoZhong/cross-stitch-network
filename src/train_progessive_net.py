...
def train(base_progress_net, classifiers, dataloader_train, dataloader_test, max_iteration, interval):
    ...
    for m_iteration in range(max_iteration):
        if m_iteration % interval == 0:
            if save_model:
                torch.save(base_progress_net.state_dict(), os.path.join(module_path['base'], 'imgnet_scratch_last_model.pth'))
                for tmp in classifiers:
                    torch.save(classifiers[tmp].state_dict(), os.path.join(module_path[tmp], 'imgnet_scratch_progress_' + tmp + 'last_model.pth'))
            correct = {}
